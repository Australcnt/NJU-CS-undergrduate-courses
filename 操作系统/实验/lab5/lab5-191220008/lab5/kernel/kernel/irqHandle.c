...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		if (((sf->edx >> 3) % 2 == 0 && destInode.type == DIRECTORY_TYPE) || ((sf->edx >> 3) % 2 == 1 && destInode.type == REGULAR_TYPE)) {
...
void syscallWrite(struct StackFrame *sf) {
	...
	if (MAX_DEV_NUM <= sf->ecx && sf->ecx < MAX_DEV_NUM + MAX_FILE_NUM) {
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	while(i < size) {
		...
		do {
			...
			i++;
		} while((remainder + i) % sBlock.blockSize != 0 && i < size);
		if (i >= size)
			break;
		if (quotient + j == inode.blockCount) {
	...
	if (quotient + j < inode.blockCount && i < size)
			readBlock(&sBlock, &inode, quotient + j, buffer);
	else if (quotient + j == inode.blockCount) {
		ret = allocBlock(&sBlock, gDesc, &inode, file[sf->ecx - MAX_DEV_NUM].inodeOffset);
		if (ret == -1) {
			diskWrite(&inode, sizeof(Inode), 1, file[sf->ecx - MAX_DEV_NUM].inodeOffset);
			inode.size = inode.blockCount * sBlock.blockSize;
...
void syscallReadFile(struct StackFrame *sf) {
	...
	if (size + file[sf->ecx - MAX_DEV_NUM].offset > inode.size)
	...
	while(i < size) {
		do {
			...
			i++;
		} while((remainder + i) % sBlock.blockSize != 0);
		readBlock(&sBlock, &inode, quotient + j, buffer);
		j++;
	}
...
void syscallLseek(struct StackFrame *sf) {
	...
	switch(sf->ebx) { // whence
		...
		case SEEK_CUR:
			// TODO: if SEEK_CUR
			if (0 <= offset + file[sf->ecx - MAX_DEV_NUM].offset && offset + file[sf->ecx - MAX_DEV_NUM].offset <= inode.size) {
		...
		case SEEK_END:
			// TODO: if SEEK_END
			if (0 <= offset + inode.size && offset + inode.size <= inode.size) {
