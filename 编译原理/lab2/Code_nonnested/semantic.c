...
Type StructSpecifier(Node* node) {
    ...
    if(!strcmp(child2->name, "OptTag")) {
        ...
        type = (Type)malloc(sizeof(struct Type_));
        type->kind = STRUCTURE;
        type->u.structure.tagName = optTag;
        type->u.structure.field = DefList(child4, CONTEXT_STRUCT);
        exitScope();
        Type optTagType = (Type)malloc(sizeof(struct Type_));
        optTagType->kind = STRUCTTAG;
        optTagType->u.structure.tagName = optTag;
        optTagType->u.structure.field = type->u.structure.field;
...
FieldList VarDec(Node* node, Type type, Context context) {
    ...
    if(!strcmp(child1->name, "ID")) {
        ...
        field->name = child1->val.val_content;
        field->type = type; // TODO: 是否需要参考symbol->type
        field->tail = NULL;
    } else {
        Node* child3 = getChild(node, 2);
        Type nextType = (Type)malloc(sizeof(struct Type_));
        nextType->kind = ARRAY;
        nextType->u.array.elem = type;
    ...
    if(!strcmp(child1->name, "Exp")) {
        ...
        if(!child3) {
            ...
            if(!symbol) {
                ...
                type = symbol->type->u.function.ret;
            }
        }
    } else {
        type = (Type)malloc(sizeof(struct Type_));
