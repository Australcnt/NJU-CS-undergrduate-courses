...
class CachingServer(TCPServer):
    ...
    def touchItem(self, path: str):
        ...
        if path in self.cacheTable and not self.cacheTable.expired(path):  
            cache_item = self.cacheTable.get(path)
            headers, body = cache_item.headers, cache_item.body
            yield headers
            yield body
            raise StopIteration
        else:
            #self.log_info(f"header returned")
            ...
            if not response is None:
                ...
                #self.log_info(f"header returned")
                yield headers
                len = response.readinto(buffer)
                #self.log_info(f"send length'{len}'")
                while len:
                    self.cacheTable.appendBody(path, buffer[0 : len])
                    #self.log_info(f"send length'{len}'")
                    yield buffer[0 : len]
                    len = response.readinto(buffer)
                raise StopIteration
            else:
                #self.log_info(f"cannot find in the main server.")
                yield headers
                yield body
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.headers = next(item)
        if self.headers is None:
            self.send_error(HTTPStatus.NOT_FOUND, "'File not found'")
        else:
            self.send_response(HTTPStatus.OK)
            self.sendHeaders(self.headers)
            '''
            self.body = next(item)
            while self.body:
                self.sendBody(self.body)
                self.body = next(item)
            '''
            while True:
                try:
